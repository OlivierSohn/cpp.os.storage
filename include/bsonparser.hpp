...
namespace imajuscule {
    namespace bsonparser {
        ...
        struct ObjectSpec {
            ...
            std::function<std::unique_ptr<Object>(Parser<platform::FileReader> &,
                                                  std::vector<std::unique_ptr<Object>>)> create_object;
        ...
        struct List : public Object {
            ...
            void logWithName(int indent, const char * name = "List", const char * elt_name = nullptr) const {
                ...
                indent -= 4;
                //logIndent(indent);
                //std::cout << "End of [" << name << " (" << elements.size() << ")]" << std::endl;
            }
        ...
        struct BinaryData : public Named {
            enum class SubType {
                GENERIC,
                FUNCTION,
                BIN_OLD,
            ...
            void log(int indent) const override {
                Named::log(indent);
                std::cout << "[Binary data (size " << data.size() << ")] ";
                for(auto d : data) {
                    std::cout << d;
                }
                std::cout << std::endl;
        ...
        static Grammar computeGrammar() {
            // The 'nullptr's below correspond to unimplemented grammar elements,
            ...
            // To make the implementaiton more straightforward, I transformed the way the grammar is formulated to an equivalent form:
            // I move the \x00 at the end of 'Document' to the 'List' definition
            
            std::map<Item, std::vector<ObjectSpec>> grammar = {
                {{Item::BYTE}, {{{}, unnamed_native<unsigned char>(), "1 byte (8-bits)"}}},
                ...
                {{Item::ELEMENT}, {
                    ...
                    {{Item::x05, Item::E_NAME, Item::BINARY}, nullptr,
                        "Binary data"},
                    
                    {{Item::x06, Item::E_NAME}, nullptr,
                    ...
                    {{Item::x0A, Item::E_NAME}, nullptr,
                        "Null value"},
                    
                    {{Item::x0B, Item::E_NAME, {Item::CSTRING, 2}}, nullptr,
                        "Regular expression (pattern, options)"},
                    
                    {{Item::x0C, Item::E_NAME, Item::STRING, {Item::BYTE, 12}}, nullptr,
                        "DBPointer — Deprecated"},
                    
                    {{Item::x0D, Item::E_NAME, Item::STRING}, nullptr,
                        "JavaScript code"},
                    
                    {{Item::x0E, Item::E_NAME, Item::STRING}, nullptr,
                        "Symbol. Deprecated"},
                    
                    {{Item::x0F, Item::E_NAME, Item::CODE_W_S}, nullptr,
                        "JavaScript code w/ scope"},
                    
                    {{Item::x10, Item::E_NAME, Item::INT32}, named_native<int32_t>(),
                        "32-bit integer" },
                    
                    {{Item::x11, Item::E_NAME, Item::UINT64}, nullptr,
                        "Timestamp"},
                    ...
                    {{Item::xFF, Item::E_NAME}, nullptr,
                        "Min key"},
                    
                    {{Item::x7F, Item::E_NAME}, nullptr,
                        "Max key"},
                }},
                ...
                {{Item::STRING}, {{{Item::INT32, {Item::BYTE, Quantification::FROM_CONTEXT}}, [](auto & reader, auto objs){
                    ...
                    for(int i=1, sz = safe_cast<int>(objs.size()); i<sz; ++i) {
                        buf.push_back(getAs<NativeUnnamed<unsigned char>>(std::move(objs[i]))->value);
                    }
                    return make_object<Str>(std::move(buf));
                }, "UTF8 String"}}},
                ...
                {{Item::BINARY}, {{{Item::INT32, Item::SUBTYPE, {Item::BYTE, Quantification::FROM_CONTEXT}}, nullptr,
                    "Binary"}}},
                
                {{Item::SUBTYPE}, {
                    {{Item::x00}, nullptr, "Generic binary subtype"},
                    {{Item::x01}, nullptr, "Function"},
                    {{Item::x02}, nullptr, "Binary (Old)"},
                    {{Item::x03}, nullptr, "UUID (Old)"},
                    {{Item::x04}, nullptr, "UUID"},
                    {{Item::x05}, nullptr, "MD5"},
                    {{Item::x80}, nullptr, "User defined"},
                }},
                
                {{Item::CODE_W_S}, {{{Item::INT32, Item::STRING, Item::DOCUMENT}, nullptr,
                    "Code w/ scope"}}},
            };
        ...
        private:
            ...
            std::unique_ptr<Object> read(Item item) {
                ...
                if(!spec.create_object) {
                    std::string desc = spec.description;
                    throw std::runtime_error("bson element with description [" + desc + "] is not yet supported");
                }
                return spec.create_object(*this, std::move(objs));
            }
            ...
            auto readComponents(std::vector<QuantifiedItem> const & objs) {
                ...
                for(auto const &i : objs) {
                    ...
                    int sz = i.getCount();
                    if(i.getQuantification() == Quantification::FROM_CONTEXT) {
                        if(!has_size) {
